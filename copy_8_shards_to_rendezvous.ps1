# Copy 8 Shards to Rendezvous Server
# Uploads shard-v2-0.gguf through shard-v2-7.gguf
...
Write-Host ""
Write-Host "Copying 8 shards to rendezvous server..." -ForegroundColor Cyan
Write-Host ""

# Find the 8 shard files (shard-v2-0 through shard-v2-7)
$shards = @()
for ($i = 0; $i -lt 8; $i++) {
    $shardPath = Join-Path $SourceDir "shard-v2-$i.gguf"
...
if ($shards.Count -eq 0) {
    Write-Host "ERROR: No shard files found (shard-v2-0.gguf through shard-v2-7.gguf)" -ForegroundColor Red
    exit 1
}
...
# Upload each shard (rename to shard-0.gguf through shard-7.gguf on server)
$uploaded = 0
$failed = 0

foreach ($shard in $shards) {
    $sizeGB = [math]::Round($shard.Length / 1GB, 2)
    $shardNum = [int]($shard.BaseName -replace 'shard-v2-', '')
    $remoteName = "shard-$shardNum.gguf"
    
    Write-Host "Uploading $($shard.Name) ($sizeGB GB) as $remoteName..." -ForegroundColor Yellow
    Write-Host "  This may take several minutes for large files..." -ForegroundColor Gray
    
    # Upload with the standard naming (shard-0.gguf, shard-1.gguf, etc.)
    $scpCmd = "scp -F NUL '$($shard.FullName)' ${RemoteUser}@${RemoteHost}:$RemoteDir/$remoteName"
