...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Load the assigned shard BEFORE joining the network
    {
        ...
        match s.load_shard_file(shard_id) {
            ...
            Err(e) => {
                eprintln!("\n[SHARD] ═══════════════════════════════════════════════════════════════════════════");
                eprintln!("[SHARD] ❌❌❌ CRITICAL: ASSIGNED SHARD {} NOT FOUND ❌❌❌", shard_id);
                eprintln!("[SHARD]   Error: {}", e);
                eprintln!("[SHARD]   Expected location: {}", s.shards_dir.join(format!("shard-{}.gguf", shard_id)).display());
                eprintln!("[SHARD]");
                eprintln!("[SHARD]   Node will NOT join the network until shard is loaded.");
                eprintln!("[SHARD]   Please ensure the shard file exists before starting the node.");
                eprintln!("[SHARD] ═══════════════════════════════════════════════════════════════════════════\n");
                
                // Exit with error - don't join network without shard
                return Err(format!("Cannot start shard listener: assigned shard {} not found. Expected: {}", 
                    shard_id, 
                    s.shards_dir.join(format!("shard-{}.gguf", shard_id)).display()
                ).into());
            }
        }
    }
                    ...
                    SwarmEvent::Behaviour(behaviour_event) => {
                        match behaviour_event {
                            ShardBehaviourEvent::Kademlia(kad::Event::RoutingUpdated { peer, .. }) => {
                                ...
                                // Only announce if shard is loaded (enforced at startup)
                                if !announced {
                                    let s = state.read().await;
                                    
                                    // Verify shard is still loaded before announcing
                                    if !s.is_shard_loaded(shard_id) {
                                        eprintln!("[DHT] ❌ Cannot announce: shard {} not loaded!", shard_id);
                                        drop(s);
                                        continue;
                                    }
                                    
                                    // Verify capabilities reflect loaded state
                                    if !s.announcement.capabilities.shard_loaded {
                                        eprintln!("[DHT] ❌ Cannot announce: shard {} capabilities not set to loaded!", shard_id);
                                        drop(s);
                                        continue;
                                    }
